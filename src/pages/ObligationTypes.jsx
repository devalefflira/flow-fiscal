...
import { 
  ...
  FileText, Calendar, Layers, CheckSquare, Clock
} from 'lucide-react';
...
export default function ObligationTypes() {
  ...
  const [obligations, setObligations] = useState([
    { 
      ...
      recurrence: 'Mensal', 
      deadlineDay: 15,
      deadlineType: 'next_month', // 15º dia útil (simplificado para dia 15 na V1)
  ...
  const getRuleText = (day, type) => {
    const period = type === 'next_month' ? 'do mês seguinte' : 'do mês corrente';
  ...
  const handleSave = () => {
    if (!formData.name.trim() || formData.regimes.length === 0) return; // Validação básica

    if (isEditing) {
      setObligations(prev => prev.map(o => o.id === formData.id ? formData : o));
    } else {
      const newId = Math.max(...obligations.map(o => o.id), 0) + 1;
      setObligations([...obligations, { ...formData, id: newId }]);
    }
    setIsModalOpen(false);
  };

  const handleDelete = (id) => {
    if (window.confirm('Excluir este tipo de obrigação?')) {
    ...
    <div className="min-h-screen bg-background p-8 font-sans flex flex-col">
      ...
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Tipos de Obrigações</h1>
              <p className="text-gray-500 text-sm">Defina regras de entrega e recorrência</p>
            </div>
        </div>
        <button 
        ...
        >
          <Plus className="w-5 h-5" /> Nova Obrigação
        </button>
      </div>
      ...
      <div className="bg-surface p-4 rounded-xl border border-white/5 mb-6">
        <div className="flex items-center gap-3 bg-background px-4 py-3 rounded-lg border border-white/10 focus-within:border-brand-cyan transition-colors">
            ...
            <input 
                type="text" 
                placeholder="Buscar obrigação (ex: DCTF, SPED)..."
                value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredObligations.map((obs) => {
          const badgeColor = getRecurrenceStyle(obs.recurrence);
          ...
          return (
            ...
            >
              ...
              <div className="p-4 border-b border-white/5 flex justify-between items-start">
                 <span className={`text-[10px] font-bold text-white uppercase px-2 py-1 rounded ${badgeColor}`}>
                   {obs.recurrence}
                 </span>
              ...
              <div className="p-5 flex-1 flex flex-col">
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {obs.regimes.map(r => (
                      <span key={r} className="px-2 py-1 rounded border border-white/10 bg-background/50 text-[10px] text-gray-300">
                        {r}
                      </span>
                    ))}
                  </div>
                ...
                {/* Regra de Prazo */}
                <div className="mt-auto flex items-center gap-2 text-gray-400 text-xs bg-white/5 p-2 rounded-lg">
                   <Clock className="w-3 h-3" />
                   <span>Vence: <strong>{getRuleText(obs.deadlineDay, obs.deadlineType)}</strong></span>
                </div>
              </div>
            </div>
          );
           ...
           <div className="bg-surface rounded-2xl w-full max-w-lg border border-white/10 shadow-2xl overflow-hidden">
              ...
              <div className="p-6 border-b border-white/10 flex justify-between items-center bg-surface">
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  ...
                  {isEditing ? 'Editar Obrigação' : 'Nova Obrigação'}
                </h2>
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                   <div className="flex items-center gap-2 mb-3 text-brand-cyan">
                      ...
                      <h3 className="font-bold text-sm">Regra de Vencimento</h3>
                   </div>
                   <div className="flex gap-3">
                      ...
                      <div className="flex-[2]">
                        <label className="text-[10px] font-bold text-gray-500 uppercase block mb-1">Quando</label>
                        <select 
                        ...
                        >
                          ...
                          <option value="same_month">Do Mês Corrente</option>
                        </select>
                      </div>
                   </div>
                </div>
                ...
                <div className="space-y-2">
                   <label className="text-xs font-bold text-gray-500 uppercase">Aplica-se aos Regimes:</label>
                        ...
                        <div 
                          ...
                          className={`
                            ...
                            ${formData.regimes.includes(regime) 
                                ...
                                : 'bg-background border-white/5 text-gray-400 hover:bg-white/5'}
                          `}
                        >
                           <div className={`w-4 h-4 rounded border flex items-center justify-center ${formData.regimes.includes(regime) ? 'border-brand-cyan bg-brand-cyan' : 'border-gray-500'}`}>
                              {formData.regimes.includes(regime) && <CheckSquare className="w-3 h-3 text-white" />}
                           </div>
                   ...
                   {formData.regimes.length === 0 && (
                     <p className="text-[10px] text-red-500 pt-1">* Selecione pelo menos um regime.</p>
                   )}
                </div>
